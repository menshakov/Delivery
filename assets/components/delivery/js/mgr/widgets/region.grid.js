Delivery.grid.Region = function (config) {
	config = config || {};
	if (!config.id) {
...
Ext.extend(Delivery.grid.Region, MODx.grid.Grid, {
	...
	createItem: function (btn, e) {
		var w = MODx.load({
			xtype: 'delivery-region-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'delivery-region-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeItem: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('delivery_region_remove')
				: _('delivery_region_remove'),
			text: ids.length > 1
				? _('delivery_region_remove_confirm')
				: _('delivery_region_remove_confirm'),
			url: this.config.url,
			params: {
	...
	getTopBar: function (config) {
		return [{
			text: '<i class="icon icon-plus">&nbsp;' + _('delivery_region_create'),
...
Ext.reg('delivery-grid-region', Delivery.grid.Region);